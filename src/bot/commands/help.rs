...
pub async fn help(ctx: Context<'_>) -> Result<(), AppError> {
    ui::send_embed(
        ...
        [
            ...
            "`/create_bond` • `/list_bonds` • `/buy_bond` • `/sell_bond` • `/offer_bond` • `/incoming_bond_offers` • `/accept_bond_offer` • `/decline_bond_offer` • `/my_bonds`",
            "",
...
pub async fn tutorial(ctx: Context<'_>) -> Result<(), AppError> {
    ui::send_embed(
        ...
        [
            ...
            "",
            "**8. Bonds**",
            "Use `/create_bond` to issue pre-funded bonds, `/buy_bond` to buy them yourself, `/offer_bond` to sell held bonds to another user, or `/sell_bond` to pitch one directly to Profit Rat.",
            "",
            "**9. Mirror Manifold**",
